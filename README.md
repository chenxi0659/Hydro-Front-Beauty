...
- 在宽度不小于 1280px 的屏幕上，将 Hydro 标准双栏页主内容区设为最大约 1152px（上一版约 1440px 的 0.8 倍），右侧栏恢复约 300px，因此主内容行最大宽度为 1452px；右侧栏不会被覆盖或移除；
- 首页右侧的讨论节点、最新题目与推荐支持独立折叠；折叠状态会保存在浏览器中，并采用内联 SVG 线条图标与平滑的先快后慢动画；
- 通过插件模板覆盖移除首页“一言”卡片，不渲染容器，也不会请求外部一言 API；
- 浏览器标签页自动移除末尾的 `-Hydro`；
- 顶部导航使用赣南师范大学校徽 Logo（桌面 Logo 高度 42px、导航高度 60px；移动端 Logo 高度 30px、导航高度 52px）；
...
校徽图片已存放在 `public/brand/gnnu-nav-logo.png`。Hydro 重启时会将插件的 `public/` 文件发布为静态资源，图片地址为 `/brand/gnnu-nav-logo.png`；插件会在页面加载后用该图片替换桌面与移动端的原 Hydro Logo。
